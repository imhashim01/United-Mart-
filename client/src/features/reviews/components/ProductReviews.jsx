...
export default function ProductReviews({ product }) {
  ...
  const breakdown = useMemo(() => {
    ...
    localReviews.forEach((r) => counts[r.rating - 1]++);
    ...
    >
        ...
        {sortedReviews.slice(0, visibleCount).map((review) => (
          <div key={review.id} className="py-5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 mt-3 text-xs text-charcoal-600 hover:text-orchard-700 transition-colors"
            >
              <ThumbsUp size={13} />
      ...
      <div className="mt-10 pt-8 border-t border-border">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 max-w-lg">
          ...
          <div>
            <label className="text-sm font-semibold text-charcoal-900 mb-1.5 block">Name</label>
            <input
              {...register("name", { required: "Name is required" })}
              placeholder="Your name"
              className={clsx(
                "w-full h-11 px-3.5 rounded-[var(--radius-sm)] border text-sm focus:outline-none focus:ring-[3px] transition-all",
                errors.name
          ...
          <button
            type="submit"
            className="self-start h-11 px-6 rounded-[var(--radius-md)] bg-orchard-900 text-white text-sm font-semibold hover:bg-orchard-700 transition-colors"
          >
            Submit Review
          </button>
        </form>
